...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{math_delta_chart_v4.png}
...
As shown in the broader MATH validation set analysis (Table \ref{tab:math}), SB-GRPO yields the most significant improvements in sub-categories requiring deep, multi-step logical derivations, such as Intermediate Algebra (+4.88\%) and Precalculus (+5.68\%). For simpler or saturated categories like Algebra (\~89\%), the semantic profiling dampens unnecessary reflections, maintaining performance parity (-0.25\%). This is largely due to the Targeted Error Correction provided by SEP, proving that K-Means clustering effectively addresses underlying systematic errors rather than just applying pattern matching.
